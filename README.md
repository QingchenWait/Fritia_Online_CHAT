...
## 2026-07-08 MCP Agent Output

- MCP 工具模式运行中不再把中间轮模型文本作为聊天正文输出；运行过程只显示在可折叠的“思考中 / MCP 调用”状态里，最终完成后才写入正式回复。
- MCP 工具产生的文件按最后一个产出文件的 MCP 步骤作为最终附件展示；更早步骤的中间文件收纳到低调的“其他文件”按钮中，点击后在悬浮窗口里按原图片、视频、音频或文件卡片方式查看和保存。
- 纯浏览器静态页面访问插件商店的 MCP 插件页时，若魔搭列表无法读取，会提示“网页版无法直接安装插件，请使用 APP 或点击下方按钮手动配置”；打包客户端保持原错误提示。
...
- “添加插件源 / 魔搭社区”会打开魔搭登录检测窗口；Tauri 桌面端会优先打开同一应用内的魔搭 WebView2 插件源窗口，窗口内提示“在本页面中完成登录”并提供“检验登录状态”按钮，主应用也会短时轮询登录态。检测到登录态后，来源菜单项前显示绿色连接圆点，并通过壳层 `modelscope_fetch` 在该窗口的同源浏览器上下文请求 hosted MCP 接口。若浏览器 CORS、登录态或 WAF 限制阻断魔搭 API，会保留错误提示和跳转官方 MCP 页的入口。
...
- MCP 插件详情窗口层级高于插件商店；“官方详情”在 Tauri 桌面端打开同一应用内的 WebView2 详情窗口，在网页/PWA 运行时回退为应用内悬浮浏览器面板，层级高于插件商店和 MCP 详情窗口。
