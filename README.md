...
- 当自定义角色与同名内置角色聊天时，聊天页右上角会提示“角色迁移”；可选择迁移、暂不迁移或不再提示。
- 迁移确认窗口会列出对话、工具调用、附件引用、长期记忆和关系数据统计，并明确说明内置角色基础信息、头像、TTS 参考语音、人格提示词和示例对话保持系统版本。
- 确认迁移后会先自动导出 ZIP 备份，再按时间顺序合并聊天与群聊发言、会话工具配置和长期记忆；只有校验全部通过后才删除自定义角色。
- 可迁移角色的资料卡“快速操作”中会显示与其他快速操作样式一致的“迁移角色”按钮；迁移成功后进度窗口保持打开并显示成功状态，点击“完成”后才关闭。
- 迁移中断或失败会在下一次启动时显示恢复窗口，不阻塞 APP 启动；可选择迁移前自动导出的 ZIP 存档进行恢复。
...
- 使用说明会读取 `src/docs` 下所有 `doc_<标题>.md` 文档；横屏为左侧文档目录 + 右侧 Markdown 内容，竖屏为目录页 / 内容页两级浏览，目录和正文都有自绘纵向滚动条。
- 网页端每次打开帮助窗口都会重新发现文档并以无缓存方式读取 Markdown；新增、删除或修改文档后，重新打开 APP 即可看到最新内容。

## 2026-07-09 v0.4.4 Quick Add Menu

- “添加”按钮弹出的悬浮下拉菜单已对齐“菜单”下拉菜单的尺寸、圆角、选项高度、图标尺寸和选项间距，桌面端与移动端使用同一套紧凑菜单规格。
- 版本更新到 `v0.4.4`，缓存批次更新到 `fritia-next-chat-v27`。

## 2026-07-09 Role Plugin Store

- “插件商店”默认进入“角色插件”分组；每次打开窗口都会从云端重新同步 `plugin_char.json?token=cyandust_workshop` 并显示加载骨架，角色卡片支持手动刷新。
- 角色插件卡片按远程目录 key 解析，显示角色名称和简介；本地已存在同名角色时安装按钮变灰禁用，并自动排到卡片组后方。
- 点击安装会逐个下载带 `token=cyandust_workshop` 的人格设定、头像、TTS 参考语音和示例对话，头像/语音保存到 IndexedDB 媒体库，角色按导入角色写入用户存档。

## 2026-07-08 Windows v0.4.3 Package

- 版本更新到 `v0.4.3`，缓存批次更新到 `fritia-next-chat-v24`。
- 本次 Windows x64 包会同步当前项目中的手动文案修改、MCP 客户端 UI 修正、Markdown 气泡和工具完整回复开关，并继续沿用 Tauri + WebView2 壳层的 CORS 代理、ModelScope 登录和 MCP relay 支持。

## 2026-07-08 MCP Client UI Reset

- “工具调用 / MCP 客户端”页重新按桌面横屏和移动竖屏两套策略布局：桌面端为服务列表 + 编辑器的稳定工作台，竖屏端为上下排列的独立 Soft UI 卡片流。
- 中等宽度横屏上下分段时由整个 MCP 客户端页面滚动，详情区保持完整形态；服务器配置 JSON 压缩到约 8 行高度并在代码框内自绘滚动；竖屏服务列表最多显示 3 项，更多服务在列表内部滚动。

## 2026-07-08 Tool Client Layout / Markdown / v0.4.1

- “工具调用 / MCP 客户端”页重新组织桌面端和竖屏端布局：服务器配置 JSON 保留最小可编辑高度，窗口高度不足时由整个 MCP 客户端页提供自绘纵向滚动条；竖屏下服务列表和编辑区上下排列，不再互相挤压。
- 日常聊天和工具调用聊天的用户 / bot 气泡支持 Markdown 渲染，包括标题、粗体、斜体、列表、引用、表格、代码块、自动链接和 `$...$` / `$$...$$` 公式文本。
- “工具调用 / 权限设置 / 对话”新增“让 bot 完整回复工具调用结果”开关，默认开启。开启后仅在工具模式请求 LLM 时追加隐藏提示，前台消息、聊天记录和长期记忆不会保存该提示。
...
- 群聊：通过单列角色多选窗口创建群聊，群聊头像固定使用 `src/_char/Profile_GroupChat.png`，群聊调度继承旧项目“圆桌密语”的单角色轮流发言与 bot-to-bot 链式接话方式。
...
- 插件商店：主菜单可打开插件商店；角色插件页从远程角色目录加载卡片并一键导入角色，MCP 插件页接入魔搭社区 hosted MCP，支持搜索、分页、详情配置，并可把远程 MCP 配置写入 Streamable HTTP MCP 服务列表。
...
- 角色导入：支持上传头像、人格设定提示词、示例对话和 TTS 参考语音；保存后角色会出现在角色列表。
...
4. 可选填写示例对话。
5. 可选上传 TTS 参考语音。
6. 保存后角色会出现在列表，可私聊，也可加入群聊。

也可以从主界面右上角“菜单 / 插件商店 / 角色插件”安装远程角色插件。角色插件会自动下载资源并写入本地用户存档；本地已存在同名角色时不会重复安装。
...
- 在搜索框中筛选角色。
- 在单列角色列表中勾选多个角色。
- 底部按钮会显示当前选择数量，至少选择 2 位角色后可创建群聊。
...
- 移动端竖屏列表页底部保留聊天、角色、导入、记忆节点和设置入口，进入聊天页时自动收起以避免遮挡输入栏。
