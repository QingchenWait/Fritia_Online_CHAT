...
MCP (Model Context Protocol，模型上下文协议) 类似一种专门给大模型使用的通用 “插件” 或 “内置 APP”。
...
- **Streamable HTTP**: 独立的 HTTP MCP 服务，更适合部署在线 MCP 服务器。
...
我们在 APP 里内置了 **插件商店**，引入 [魔搭社区](https://www.modelscope.cn/mcp?hosted=1) 的在线 MCP 插件源，可以方便地进行一键部署。

1. 点击头像旁边的**菜单**图标 (三条横线)，并选择 [插件商店] 选项。
2. 进入插件商店页面后，选择 [MCP 插件] 选项，此时会出现魔搭社区的 MCP 服务器列表，但**需要登录**才能使用。
3. 注册/登录魔搭社区：点击页面中的 [添加插件源]，选择 [魔搭社区]，会弹出魔搭社区的官方网站，操作网站进行注册/登录操作。
4. 登录成功后，官网页面会自动退出，点击 `刷新` 按钮，即可查看并部署在线 MCP 服务器。


## 🌟 手动调用在线 MCP 服务器

除了 [魔搭社区](https://www.modelscope.cn/mcp?hosted=1)，还可以使用一些提供在线 MCP 服务的厂商，例如：[腾讯云](https://cloud.tencent.com/developer/mcp)、[阿里云百炼](https://bailian.console.aliyun.com/#/mcp-market) 等，进行手动部署。

点击 “MCP 客户端” 栏目，新建一个 `Streamable HTTP` 客户端，“服务器名称” 可以自己随便取。

在 “服务器配置 JSON” 中，输入类似以下格式的配置，一般可以在在线 MCP 服务器的详情页面里找到：
